<!-- Header & imports -->
<%@page contentType="text/html" pageEncoding="UTF-8" %>
<%@include file="../../includes/header.inc" %>
<!-- End header -->

<!-- Container -->
<div class="container min-vh-100">
    <!-- Row -->
    <div class="row">

        <!-- Main -->
        <main role="main">
            <!-- Section -->
            <section class="col-12">

                <!-- Row -->
                <div class="row justify-content-center">

                    <div class="my-5 text-center">
                        <h1 class="text-primary">Carport med standard mål</h1>
                    </div>

                    <!-- Group cards -->
                    <div class="card-group mb-5">
                        <!-- Left card -->
                        <section class="card border-dark shadow mr-3">
                            <div class="card-body d-flex flex-column">
                                <img class="card-img img-fluid" src="images/filler.png" alt="Standard byg">
                                <h2 class="card-title my-3">Carport enkelt </h2>
                                <h5 class="card-subtitle"> CPO01HR<br/>Enkelt carport på 3,90x7,80M med redskabsrum på 2,40x3,30M </h5>
                                <br/>
                                <p class="card-text">Enkelt carport med høj rejsning. 3,90 x 7,80 m.
                                    <br/>Extra bred model.
                                    3,30 x 2,40 m redskabsrum.
                                    <br/>Højde; 3,10 m.
                                    <br/>Trykimprægnerede stolper, stern og beklædning.
                                    <br/>Leveres med: Søm, skruer, beslag og betontagstenstag.
                                    <br/>Udførlig byggevejledning til carport og spær medfølger.
                                    <br/>Betontagsten i sort med 30 års garanti.
                                    <br/><br/>NB! Leveres som Byg-selv sæt - usamlet og ubehandlet!
                                </p>
                                <a class="card-link btn btn-dark btn-block mt-auto"
                                   href="FrontController?target=redirect&destination=customerCarportStandard">Bestil -
                                    pr. stk. 23.998,-</a>
                            </div>
                        </section>
                        <!-- End left card -->
                        <!-- Right card -->
                        <section class="card border-dark shadow ml-3">
                            <div class="card-body d-flex flex-column">
                                <img class="card-img img-fluid" src="images/flatroof.png" alt="Standard byg">
                                <h2 class="card-title my-3">Carport enkelt </h2>
                                <h5 class="card-subtitle"> 00816<br/>CARPORT ENKELT 3,00X4,80M CAR01 FLADT TAG </h5>
                                <br/>
                                <p class="card-text">Enkelt carport med høj rejsning. 3,90 x 7,80 m.
                                    <br/>3,00 x 4,80 mtr.

                                    <br/>Højde; 2,25 mtr.
                                    <br/>Trykimprægnerede stolper, stern og beklædning.
                                    <br/>Leveres med: søm, skruer, beslag og plasttrapez tag m/bundskruer.
                                    <br/>Udførlig byggevejledning til carport medfølger.
                                    <br/><br/>NB! Leveres som Byg-selv sæt - usamlet og ubehandlet!
                                </p>
                                <a class="card-link btn btn-dark btn-block mt-auto"
                                   href="FrontController?target=redirect&destination=customerCarportStandard">Bestil -
                                    pr. stk. 23.998,-</a>
                            </div>
                        </section>
                        <!-- End right card -->
                    </div>
                    <!-- End group cards -->

                </div>
                <!-- End row -->

            </section>
            <!-- End section -->
        </main>
        <!-- End main -->
    </div>
    <!-- End row -->
</div>
<!-- End container -->

<!-- Footer -->
<%@include file="../../includes/footer.inc" %>
<!-- End footer -->

